...
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Exception\IrrecoverableDownloadException;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Package\Comparer\Comparer;
...
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Util\Url as UrlUtil;
    ...
    public function download(PackageInterface $package, string $path, ?PackageInterface $prevPackage = null, bool $output = true): PromiseInterface
    {
        ...
        $download = function () use ($output, $cacheKeyGenerator, $package, $fileName, &$urls, &$accept, &$reject) {
            ...
            if ($this->cache !== null && ($checksum === null || $checksum === '' || $checksum === $this->cache->sha1($cacheKey)) && $this->cache->copyTo($cacheKey, $fileName)) {
                ...
                $result = \BracketSpace\Notification\BuddyPress\Dependencies\React\Promise\resolve($fileName);
            } else {
                if ($output) {
