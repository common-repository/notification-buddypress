...
namespace BracketSpace\Notification\BuddyPress\Dependencies\Composer\Console;

use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Installer;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\IO\NullIO;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Util\Filesystem;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Util\Platform;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Util\Silencer;
use LogicException;
use RuntimeException;
use BracketSpace\Notification\BuddyPress\Dependencies\Seld\Signal\SignalHandler;
...
use BracketSpace\Notification\BuddyPress\Dependencies\Seld\JsonLint\ParsingException;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Command;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Composer;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Factory;
...
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\IO\ConsoleIO;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Json\JsonValidationException;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Util\ErrorHandler;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Util\HttpDownloader;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\EventDispatcher\ScriptExecutionException;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Exception\NoSslException;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\XdebugHandler\XdebugHandler;
use BracketSpace\Notification\BuddyPress\Dependencies\Symfony\Component\Process\Exception\ProcessTimedOutException;
use BracketSpace\Notification\BuddyPress\Dependencies\Composer\Util\Http\ProxyManager;
...
';
    ...
    public function __construct(string $name = 'BracketSpace\Notification\BuddyPress\Dependencies\Composer', string $version = '')
    {
            ...
            if (is_file($file) && Filesystem::isReadable($file) && is_array($composer = json_decode(file_get_contents($file), true))) {
                if (isset($composer['scripts']) && is_array($composer['scripts'])) {
                    foreach ($composer['scripts'] as $script => $dummy) {
                        if (!defined('BracketSpace\Notification\BuddyPress\Dependencies\Composer\Script\ScriptEvents::'.str_replace('-', '_', strtoupper($script)))) {
                            if ($this->has($script)) {
     ...
     * @return \BracketSpace\Notification\BuddyPress\Dependencies\Symfony\Component\Console\Command\Command[]
     */
    ...
    private function getPluginCommands(): array
    {
        ...
        if (null !== $composer) {
            ...
            foreach ($pm->getPluginCapabilities('BracketSpace\Notification\BuddyPress\Dependencies\Composer\Plugin\Capability\CommandProvider', ['composer' => $composer, 'io' => $this->io]) as $capability) {
